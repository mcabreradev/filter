...
#### Scenario: filterSync is absent from the public exports

- **WHEN** a user inspects the named exports of `packages/filter/src/index.ts`
- **THEN** `filterSync` is not among them and the function is only reachable internally by the TS facade
